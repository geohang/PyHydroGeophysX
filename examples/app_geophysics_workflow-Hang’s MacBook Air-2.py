...
def render_seismic_processing_tab(sidebar_state: Dict[str, Any]) -> None:
    """Render raw SEG-Y preprocessing, first-break picking, and SRT launch UI."""

    st.markdown("---")
    st.subheader("Raw SEG-Y to SRT")
    st.caption("Local workflow for SEG-Y shot gathers: preprocess, pick first breaks, export travel-time data, then run SRT.")
    ...
    def _pick_row_for_click(
        ...
        source_x = float(header.source_x if np.isfinite(header.source_x) and header.source_x else source_id)
        receiver_x = float(header.receiver_x if np.isfinite(header.receiver_x) and header.receiver_x else selected_gather.offsets[trace_pos])
    ...
    def _update_remaining_from_manual_pattern(
        ...
        if manual_count < 2:
            return picks_work, manual_count, 0, "At least two manual picks are needed to learn a travel-time trend."

        manual_x = local_positions[manual_mask].astype(float)
        manual_t = pick_times[manual_mask].astype(float)
        order = np.argsort(manual_x)
        manual_x = manual_x[order]
        manual_t = manual_t[order]
        degree = 2 if manual_count >= 4 else 1
        try:
            coeffs = np.polyfit(manual_x, manual_t, deg=degree)
        except (TypeError, ValueError, np.linalg.LinAlgError) as exc:
            return picks_work, manual_count, 0, f"Could not fit the manual pick pattern: {exc}"
        ...
        manual_traces = {int(round(value)) for value in manual_x if np.isfinite(value)}

        updated_count = 0
        for trace_pos in range(n_traces):
            if trace_pos in manual_traces:
                continue
            predicted = float(np.polyval(coeffs, float(trace_pos)))
            ...
            if hi <= lo:
                lo = int(np.clip(center, 0, max_sample))
                hi = min(max_sample + 1, lo + 1)
            window = trace_data[lo:hi, trace_pos]
            finite = np.isfinite(window)
            if finite.any():
                scores = np.where(finite, np.abs(window), -np.inf)
                sample_idx = lo + int(np.argmax(scores))
            else:
                sample_idx = center
    ...
    with source_col:
        segy_upload = st.file_uploader(
            "SEG-Y file",
            type=["sgy", "segy"],
            help="Upload a raw SEG-Y shot-gather file.",
        )
        segy_path_text = st.text_input(
            "Or use a local SEG-Y path",
            value=default_path,
            help="Local path is useful for large files that should not be uploaded through the browser.",
        )
    with settings_col:
        ...
        st.number_input(
            ...
            min_value=0.01,
            step=0.5,
            value=float(st.session_state.get("seismic_receiver_spacing") or 1.0),
    ...
    if st.button("Load SEG-Y", type="primary", use_container_width=True):
        try:
            if segy_upload is not None:
                temp_dir = Path(tempfile.mkdtemp(prefix="phgx_segy_"))
                segy_path = save_upload(segy_upload, temp_dir)
            else:
                segy_path = Path(segy_path_text).expanduser()
            if not segy_path.exists():
                st.error(f"SEG-Y file not found: {segy_path}")
                return
            with st.spinner("Reading SEG-Y headers and traces..."):
                dataset = read_segy(str(segy_path), max_traces=None if load_all else int(max_traces))
            ...
            st.error(f"Could not read SEG-Y file: {exc}")
            return
    ...
    if processed is not None:
        ...
        if tool_cols[4].button("-1 sample", use_container_width=True):
            _nudge_active_pick(gather, -1)
        if tool_cols[5].button("+1 sample", use_container_width=True):
            _nudge_active_pick(gather, 1)
        if tool_cols[6].button("Clear picks", use_container_width=True, disabled=picks_df is None or picks_df.empty):
            _push_pick_history()
    ...
    with pick_col:
        if processed is not None:
            ...
            st.caption("Select a trace, slide the travel time while watching the plot, then save it as a manual pick.")
            ...
            manual_trace = int(
                st.number_input(
                    ...
                    value=trace_default,
                    step=1,
                    key=f"seismic_manual_trace_{selected_record}",
            ...
            with st.expander("Assisted learning", expanded=True):
                learn_window = float(
                    st.number_input(
                        "Search half-window (s)",
                        min_value=dt_step,
                        max_value=max(float(max_pick_time), dt_step),
                        value=min(0.008, max(dt_step, float(max_pick_time))),
                        step=dt_step,
                        format="%.6f",
                        help="After fitting the manual-pick trend, search this half-window for the strongest nearby break.",
                    )
                )
                auto_learn_after_manual = st.checkbox(
                    "Update after save",
                    value=True,
                    help="When at least two manual picks exist, update non-manual picks after each manual edit.",
                )
            if st.button("Set manual pick", type="primary", use_container_width=True, key=f"seismic_set_manual_{selected_record}"):
                _push_pick_history()
                ...
                if auto_learn_after_manual:
                    ...
                    if learn_error and learned_manual_count >= 2:
                        st.warning(learn_error)
                    elif learned_count > 0:
                        updated_df = learned_df
                st.session_state.seismic_active_trace = int(manual_trace)
                st.session_state.seismic_active_time = float(manual_time)
            ...
            if col_dec.button(
                ...
                key=f"seismic_pick_panel_dec_{selected_record}",
            ):
                st.session_state.seismic_active_trace = int(manual_trace)
                st.session_state.seismic_active_time = max(0.0, float(manual_time) - dt_step)
                _bump_manual_picker_version()
                st.rerun()
            if col_inc.button(
                "+1 sample",
                use_container_width=True,
                disabled=manual_picks_df is None,
                key=f"seismic_pick_panel_inc_{selected_record}",
            ):
                st.session_state.seismic_active_trace = int(manual_trace)
                st.session_state.seismic_active_time = min(float(max_pick_time), float(manual_time) + dt_step)
                _bump_manual_picker_version()
                st.rerun()
            if st.button(
                "Delete trace pick",
            ...
            if st.button(
                ...
                use_container_width=True,
                disabled=manual_count < 2 or manual_picks_df is None or manual_picks_df.empty,
                help="Fit the manual picks and update only auto/learned traces.",
                ...
                if learn_error:
                    ...
                    st.rerun()
            st.caption(f"Manual anchors: {manual_count}")
    ...
    with plot_col:
        if processed is not None:
            ...
            if plot_picks_df is not None and not plot_picks_df.empty:
                ...
                other_mask = valid & ~np.isin(pick_sources, ["manual", "learned", "auto"])
                if other_mask.any():
                    fig.add_trace(
                        go.Scatter(
                            x=pick_x[other_mask],
            ...
            fig.update_layout(
                ...
                dragmode="pan",
                hovermode="closest",
                xaxis={"range": [-0.5, float(traces.shape[1] - 0.5)]},
                yaxis={"range": [float(plot_time[-1]), 0.0]},
                uirevision=f"manual-{processed['field_record']}-{manual_trace}-{manual_time}",
            )
            ...
            st.info("Load a SEG-Y file, select a gather, then run processing to preview picks.")

    if picks_df is not None and not picks_df.empty:
        st.markdown("#### Review Picks")
        disabled_pick_columns = ["source_id", "receiver_id", "field_record", "trace_number", "trace_index", "amplitude"]
...
def main() -> None:
    ...
    tab_workflow, tab_seismic, tab_hydro_multi, tab_tutorial, tab_concepts, tab_local, tab_author = st.tabs([
        "🚀 Run Workflow",
        "Seismic Processing",
        "🌊 Hydro → Geophysics",
