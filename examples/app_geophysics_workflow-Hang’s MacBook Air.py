...
def _render_surface_picker(data_dir: Path) -> None:
    ...
    try:
        ...
        # Keep profile points within map bounds so markers remain visible.
        max_x = max(0, top.shape[1] - 1)
        max_y = max(0, top.shape[0] - 1)
        p1x = float(np.clip(st.session_state.get("hydro_point1_x", 115.0), 0, max_x))
        p1y = float(np.clip(st.session_state.get("hydro_point1_y", 70.0), 0, max_y))
        p2x = float(np.clip(st.session_state.get("hydro_point2_x", 95.0), 0, max_x))
        p2y = float(np.clip(st.session_state.get("hydro_point2_y", 180.0), 0, max_y))
        st.session_state.hydro_point1_x = p1x
        st.session_state.hydro_point1_y = p1y
        st.session_state.hydro_point2_x = p2x
        st.session_state.hydro_point2_y = p2y
        p1 = [p1x, p1y]
        p2 = [p2x, p2y]
        fig.add_trace(
            go.Scatter(
...
def _get_planned_methods() -> List[str]:
    ...
    if st.session_state.hydro_run_style == "Single method":
        return [st.session_state.hydro_single_method]
...
def _hydro_step_status() -> Dict[str, Any]:
    ...
    profile_ok = root is not None and Path(root).exists()
...
def _render_method_step() -> None:
    """Step 2 - Method selection with visual pills."""
    st.markdown("##### Select geophysical methods")
    st.caption("Click methods to toggle them on or off, or use the presets below.")

    # Real Streamlit controls
    st.radio(
        "Mode",
    ...
    if st.session_state.hydro_run_style == "Single method":
        st.selectbox("Method", options=HYDRO_RESPONSE_METHODS, key="hydro_single_method")
    else:
        # Preset buttons
        p1, p2, p3, p4 = st.columns(4)
        if p1.button("All", key="hydro_btn_all3", use_container_width=True):
        ...
        if p3.button("EM + Gravity", key="hydro_btn_emg3", use_container_width=True):
            ...
            st.rerun()
        if p4.button("Clear", key="hydro_btn_clear3", use_container_width=True):
    ...
    methods = _get_planned_methods()
    if not methods:
        st.error("Select at least one method to proceed.")

    # Advanced settings — only show sections relevant to selected methods
    with st.expander("Advanced: Acquisition & Rock Physics", expanded=False):
        _render_advanced_settings(methods)

    with st.expander("Natural Language Dialog Control", expanded=False):
        _render_dialog_control()


def _render_advanced_settings(methods: Optional[List[str]] = None) -> None:
    """Render acquisition + rock physics parameter controls for selected methods."""
    if methods is None:
        methods = _get_planned_methods()
    method_set = set(methods)

    has_ert = "ERT" in method_set
    has_srt = "SRT" in method_set
    has_em_grav = bool(method_set & {"TDEM", "FDEM", "Gravity"})
    has_profile = "Profile" in method_set or has_ert or has_srt  # profile is implied
    ...
    # --- Profile & Sampling ---
    if has_profile or has_em_grav:
        st.markdown("**Profile & Sampling**")
    ...
    # --- Experiment Setup (only show sections for selected methods) ---
    if has_ert or has_srt:
        st.markdown("**Experiment Setup**")
        if has_ert and has_srt:
        ...
        if has_ert:
            with ex_ert:
                ...
                with ec1:
                    st.number_input("Electrodes", min_value=4, step=1, key="hydro_ert_num_electrodes")
                with ec2:
                    st.number_input("Spacing (m)", min_value=0.2, step=0.1, key="hydro_ert_electrode_spacing")
                st.number_input("Start X (m)", step=0.5, key="hydro_ert_electrode_start")
        ...
        if has_srt:
            with ex_srt:
                ...
                with sc1:
                    st.number_input("Sensors", min_value=4, step=1, key="hydro_srt_num_sensors")
                with sc2:
                    st.number_input("Spacing (m)", min_value=0.2, step=0.1, key="hydro_srt_sensor_spacing")
                st.number_input("Start X (m)", step=0.5, key="hydro_srt_sensor_start")
                st.number_input("Shot interval", min_value=1, step=1, key="hydro_srt_shot_distance")

    # --- Rock Physics (only show tabs relevant to selected methods) ---
    # ERT / TDEM / FDEM => Resistivity model
    # SRT => Velocity model
    # Gravity => Density model
    needs_resistivity = bool(method_set & {"ERT", "TDEM", "FDEM"})
    needs_velocity = has_srt
    needs_density = "Gravity" in method_set

    rp_tabs_names: List[str] = []
    if needs_resistivity:
        rp_tabs_names.append("Resistivity model")
    if needs_velocity:
        rp_tabs_names.append("Velocity model")
    if needs_density:
        rp_tabs_names.append("Density model")

    if rp_tabs_names:
        st.markdown("**Rock Physics**")
        rp_tabs = st.tabs(rp_tabs_names)
        tab_idx = 0

        if needs_resistivity:
            with rp_tabs[tab_idx]:
                st.caption("Layer-wise electrical parameters for hydro-to-resistivity conversion.")
                rp1, rp2, rp3 = st.columns(3)
                with rp1:
                    st.markdown("*Top*")
                    st.number_input("rho_sat", min_value=1.0, step=10.0, key="hydro_rho_sat_top")
                    st.number_input("Archie n", min_value=0.1, step=0.1, key="hydro_archie_n_top")
                    st.number_input("sigma_s", min_value=0.0, step=0.0005, format="%.4f", key="hydro_sigma_s_top")
                with rp2:
                    st.markdown("*Middle*")
                    st.number_input("rho_sat", min_value=1.0, step=10.0, key="hydro_rho_sat_mid")
                    st.number_input("Archie n", min_value=0.1, step=0.1, key="hydro_archie_n_mid")
                    st.number_input("sigma_s", min_value=0.0, step=0.0005, format="%.4f", key="hydro_sigma_s_mid")
                with rp3:
                    st.markdown("*Bottom*")
                    st.number_input("rho_sat", min_value=1.0, step=10.0, key="hydro_rho_sat_bot")
                    st.number_input("Archie n", min_value=0.1, step=0.1, key="hydro_archie_n_bot")
                    st.number_input("sigma_s", min_value=0.0, step=0.0005, format="%.4f", key="hydro_sigma_s_bot")
            tab_idx += 1

        if needs_velocity:
            with rp_tabs[tab_idx]:
                st.caption("Layer-wise elastic parameters for synthetic velocity modeling.")
                sv1, sv2, sv3 = st.columns(3)
                with sv1:
                    st.markdown("*Top*")
                    st.number_input("Bulk mod.", min_value=1.0, step=1.0, key="hydro_top_bulk_modulus")
                    st.number_input("Shear mod.", min_value=1.0, step=1.0, key="hydro_top_shear_modulus")
                    st.number_input("Density", min_value=500.0, step=10.0, key="hydro_top_mineral_density")
                    st.number_input("Depth factor", min_value=0.1, step=0.1, key="hydro_top_depth")
                with sv2:
                    st.markdown("*Middle*")
                    st.number_input("Bulk mod.", min_value=1.0, step=1.0, key="hydro_mid_bulk_modulus")
                    st.number_input("Shear mod.", min_value=1.0, step=1.0, key="hydro_mid_shear_modulus")
                    st.number_input("Density", min_value=500.0, step=10.0, key="hydro_mid_mineral_density")
                    st.number_input("Aspect ratio", min_value=0.001, max_value=1.0, step=0.01, key="hydro_mid_aspect_ratio")
                with sv3:
                    st.markdown("*Bottom*")
                    st.number_input("Bulk mod.", min_value=1.0, step=1.0, key="hydro_bot_bulk_modulus")
                    st.number_input("Shear mod.", min_value=1.0, step=1.0, key="hydro_bot_shear_modulus")
                    st.number_input("Density", min_value=500.0, step=10.0, key="hydro_bot_mineral_density")
        ...
        if needs_density:
            with rp_tabs[tab_idx]:
                ...
                with gd1:
                    st.markdown("*Top*")
                    st.number_input("Grain density (kg/m³)", min_value=500.0, step=10.0, key="hydro_grav_density_top")
                    st.number_input("Pore-fluid density", min_value=500.0, step=10.0, key="hydro_grav_fluid_density_top")
                with gd2:
                    st.markdown("*Middle*")
                    st.number_input("Grain density (kg/m³)", min_value=500.0, step=10.0, key="hydro_grav_density_mid")
                    st.number_input("Pore-fluid density", min_value=500.0, step=10.0, key="hydro_grav_fluid_density_mid")
                with gd3:
                    st.markdown("*Bottom*")
                    st.number_input("Grain density (kg/m³)", min_value=500.0, step=10.0, key="hydro_grav_density_bot")
                    st.number_input("Pore-fluid density", min_value=500.0, step=10.0, key="hydro_grav_fluid_density_bot")
            tab_idx += 1
...
def _render_profile_step() -> None:
    ...
    st.caption("Click on the surface map to select P1 and P2, or enter coordinates manually below.")

    # Surface map first (hero element)
    root = st.session_state.get("hydro_root")
    ...
    with coord_col:
        ...
        with pc1:
            p1x = st.number_input(
                "P1 X",
                value=float(st.session_state.get("hydro_point1_x", 115.0)),
                step=1.0,
                format="%.2f",
            )
        with pc2:
            p1y = st.number_input(
                "P1 Y",
                value=float(st.session_state.get("hydro_point1_y", 70.0)),
                step=1.0,
                format="%.2f",
            )
        with pc3:
            p2x = st.number_input(
                "P2 X",
                value=float(st.session_state.get("hydro_point2_x", 95.0)),
                step=1.0,
                format="%.2f",
            )
        with pc4:
            p2y = st.number_input(
                "P2 Y",
                value=float(st.session_state.get("hydro_point2_y", 180.0)),
                step=1.0,
                format="%.2f",
            )
    with snap_col:
    ...
    with reset_col:
        ...
        if st.button("Reset", key="hydro_reset_points", use_container_width=True):
            st.session_state.hydro_surface_selected_points = []
            st.session_state.hydro_point1_x = 115.0
            st.session_state.hydro_point1_y = 70.0
            st.session_state.hydro_point2_x = 95.0
            st.session_state.hydro_point2_y = 180.0
            st.session_state.hydro_snapshot_index = 5
            st.rerun()

    # Persist profile controls in plain session keys to survive step navigation.
    ...
    p1 = (st.session_state.hydro_point1_x, st.session_state.hydro_point1_y)
    p2 = (st.session_state.hydro_point2_x, st.session_state.hydro_point2_y)
    st.caption(f"P1 = ({p1[0]}, {p1[1]})  &rarr;  P2 = ({p2[0]}, {p2[1]})  |  Snapshot = {st.session_state.hydro_snapshot_index}")
