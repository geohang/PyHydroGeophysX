...
def positive_test (Dcurve, DecayTime): 
    ...
    for i in range(np.shape(Dcurve)[0]):     
        DecayCoefs = np.linalg.lstsq(np.vstack([DecayTime, np.ones(len(DecayTime))]).T,Dcurve.iloc[i,:], rcond=-1)[0]
        DC_slope[i]=DecayCoefs[0]
    return DC_slope

def linear_coefs (x0, y0): #linear fit parameteres for decay curve
    i2keep = (x0 > 0) & (y0 > 0)
    x = x0[i2keep]
    y = y0[i2keep]
    ...
    if np.log(y).sum() !=0:
        ...
        coefs = np.linalg.lstsq(np.vstack([data_no_nan[:,0], np.ones(len(data_no_nan[:,0]))]).T,data_no_nan[:,1], rcond=-1)[0]
        return coefs
    else:
        coefs = np.array([0,0])
        return
...
def DCA(data_in, dump=None): 
    ...
    for i in range(len(filtered_R_IP['ip'])): #calculating decay curve fit parameteres - m=at^b (m: chargeability, t: time, a and b: fitting parameters)
        DC_fit_linear[i,:] = linear_coefs(DecayTime,filtered_R_IP[['M1', 'M2', 'M3', 'M4', 'M5', 'M6', 'M7', 'M8', 'M9',
                    'M10', 'M11', 'M12', 'M13', 'M14', 'M15', 'M16', 'M17', 'M18', 'M19', 'M20']].iloc[i,:].values)            
    ...
    fit_DC = pd.DataFrame(filtered_R_IP['DC_fit_a'].values[:,None]*DecayTime**filtered_R_IP['DC_fit_b'].values[:,None]).rename(columns = {0:'Fit_DC_1', 1:'Fit_DC_2', 2:'Fit_DC_3', 3:'Fit_DC_4',4:'Fit_DC_5', 5:'Fit_DC_6', 6:'Fit_DC_7', 7:'Fit_DC_8',
    ...
    temp_DC_master = pd.concat([filtered_R_IP[['An','Bn']],(pd.DataFrame(fit_DC.values*filtered_R_IP['weight'].values[:,None]))],axis=1)
    master_DC = pd.DataFrame((temp_DC_master.groupby(['An','Bn']).sum()).values/(filtered_R_IP.groupby(['An','Bn'])['weight']).sum().values[:,None])
    ...
    for i in range(len(filtered_R_IP.groupby(['An','Bn']))):
        ...
        for j in range(len(group)):
            errors = np.array([np.mean((master_DC_indexed.iloc[i,:-1].values[:,None] - group[['M1', 'M2', 'M3', 'M4', 'M5', 'M6', 'M7', 'M8', 'M9',
                                        'M10', 'M11', 'M12', 'M13', 'M14', 'M15', 'M16', 'M17', 'M18', 'M19', 'M20']].iloc[j,:].values[:,None] + m)) for m in shift])
