...
class JointERTSRTInversion(InversionBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        defaults = {
            ...
            "ert_relative_error": 0.03,
            "srt_relative_error": 0.03,
    ...
    def _build_ert_observed_log_data(self) -> np.ndarray:
        if "rhoa" in self.ert_data.dataMap():
            rhoa = np.asarray(self.ert_data["rhoa"], dtype=float).ravel()
        else:
            if "k" not in self.ert_data.dataMap():
    ...
    def _stack_system(
        ...
        Bm = B.dot(m)
        WdJ = Wd.dot(J)
        LmWm = float(lambda_model) * Wm
        LcB = float(lambda_cg) * csr_matrix(B)
    ...
    def _solve_linear_update(self, A: csr_matrix, rhs: np.ndarray) -> np.ndarray:
        method = str(self.parameters.get("solver", "lsqr")).lower().strip()
        if method == "scipy_lsqr":
            dm = lsqr(
                A,
                rhs,
                atol=float(self.parameters.get("solver_tol", 1e-8)),
                btol=float(self.parameters.get("solver_tol", 1e-8)),
                iter_lim=int(self.parameters.get("solver_maxiter", 400)),
            )[0]
    ...
    def _line_search(
        ...
        directional = float(np.asarray(dm).reshape(1, -1).dot(np.asarray(grad).reshape(-1, 1)))
    ...
    def run(self) -> JointERTSRTResult:
        ...
        for iteration in range(max_iter):
            ...
            A_s, rhs_s, g_s, phi_d_s, phi_m_s, phi_cg_s = self._stack_system(
                ...
                lambda_model=float(self.parameters["lambda_srt"]),
            ...
            A_r, rhs_r, g_r, phi_d_r, phi_m_r, phi_cg_r = self._stack_system(
                ...
                lambda_model=float(self.parameters["lambda_ert"]),
            ...
            if (chi2_ert < target_chi2 and chi2_srt < target_chi2) or (dphi_ert < dphi_tol and dphi_srt < dphi_tol):
                break
