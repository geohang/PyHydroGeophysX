...
class _FakeProvider:
    ...
    def __init__(self, scripted, vision: bool = False):
...
def _example_sgy():
    """Path to the bundled seismic example, or ``None`` in a trimmed checkout."""
    from pathlib import Path

    path = (Path(__file__).resolve().parents[3]
            / "examples" / "data" / "Seismic" / "AP_411.sgy")
...
def main() -> int:
    ...
    # 8b. The window must fit a common screen. Every module page has been built
    #     by now, so the stack's minimum reflects the widest of them. The window
    #     has to be shown first: an unshown window reports a minimum its laid-out
    #     self never uses, which is not the number the user runs into.
    window.show()
    for _ in range(20):
        app.processEvents()
    min_w = window.minimumSizeHint().width()
    ok &= _check(
        "window minimum fits a 1920 px screen",
        min_w <= 1860, f"minimum width={min_w} (was 2699 before the relax pass)",
    )

    # 9. Vision: capture_view renders a real panel and reaches the model as an image.
    controller.dispatch("navigate", {"module": "seismic"})
    shot = controller.dispatch("capture_view", {})
    img = shot.get("_image") or {}
    ok &= _check(
        "capture_view renders a panel",
        shot.get("status") == "ok" and len(img.get("data") or "") > 500,
        f"status={shot.get('status')} view={shot.get('view')} bytes={len(img.get('data') or '')}",
    )
    views = controller.dispatch("describe_current_module", {}).get("describe", {}).get("views")
    ok &= _check("describe reports capturable views", bool(views), f"views={views}")
    ok &= _check("unknown view -> failed",
                 controller.dispatch("capture_view", {"view": "nope"}).get("status") == "failed")
    # A module with no context hook, and one whose data is absent, still capture.
    ok &= _check("capture without a context hook still succeeds",
                 "context" not in controller.dispatch("capture_view", {"view": "page"}))

    # 9b. Numbers travel with the picture: the gather ships its pick table.
    sgy = _example_sgy()
    if sgy is None:
        print("[SKIP] gather context - example SEG-Y not found")
    else:
        controller.dispatch("apply_action", {"action": "load_data", "args": {"path": sgy}})
        controller.dispatch("apply_action", {"action": "set_geometry", "args": {
            "spacing": 1.0, "geophone_start": 0.0, "first_shot_x": 44.0, "shot_spacing": 1.0}})
        controller.dispatch("apply_action", {"action": "auto_pick", "args": {}})
        gather = controller.dispatch("capture_view", {"view": "gather"})
        ctx = gather.get("context") or {}
        picks = ctx.get("pick_times_ms") or {}
        ok &= _check(
            "gather capture ships exact pick times",
            gather.get("status") == "ok" and len(picks) > 5
            and "suspect_traces" in ctx,
            f"picks={len(picks)} suspect={ctx.get('suspect_traces')}",
        )

    # 10. The tool is gated on the model, and a capture leaves the transcript as
    #     one contiguous tool block followed by a separate image user message.
    verdict = {"content": "The gather looks clean.", "tool_calls": []}
    text_panel = AquahChatPanel(controller, provider=_FakeProvider([], vision=False))
    vision_panel = AquahChatPanel(controller, provider=_FakeProvider([verdict], vision=True))
    text_tools = {s["name"] for s in text_panel._tool_specs}
    vision_tools = {s["name"] for s in vision_panel._tool_specs}
    ok &= _check("capture_view gated on model vision",
                 "capture_view" not in text_tools and "capture_view" in vision_tools,
                 f"text={sorted(text_tools)} vision={sorted(vision_tools)}")

    vision_panel._set_busy(True)
    vision_panel._on_llm_ok({
        "content": "Let me look at the gather.",
        "tool_calls": [{"id": "v1", "name": "capture_view", "arguments": {}}],
    })
    vision_panel._on_approve()
    msgs = vision_panel._messages
    tool_msg = next((m for m in msgs if m.get("role") == "tool" and m.get("id") == "v1"), None)
    img_msg = next((m for m in msgs if m.get("role") == "user"
                    and providers.has_image(m.get("content"))), None)
    no_b64_in_text = tool_msg is not None and "_image" not in (tool_msg.get("content") or {})
    ok &= _check(
        "capture reaches the model as an image, not as base64 text",
        img_msg is not None and no_b64_in_text and msgs.index(img_msg) > msgs.index(tool_msg),
        f"tool={tool_msg is not None} image={img_msg is not None} clean={no_b64_in_text}",
    )
    if vision_panel._worker is not None:
        vision_panel._worker.wait(2000)

    # 11. The image budget drops older figures before they are re-sent.
    windowed = providers.window_messages(msgs + [dict(img_msg), dict(img_msg)], max_images=1)
    with_images = [m for m in windowed if providers.has_image(m.get("content"))]
    ok &= _check("context keeps only the newest figures", len(with_images) == 1,
                 f"kept={len(with_images)}")
