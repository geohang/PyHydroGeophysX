...
from PyHydroGeophysX.qt_apps import hydro_pipeline, io_utils, theme
...
class ForwardWorker(QThread):
    """Runs the hydro->geophysics forward pipeline off the UI thread."""

    logged = Signal(str, str)
    succeeded = Signal(dict)
    failed = Signal(str, bool)

    def __init__(self, context, params, methods, point1, point2) -> None:
        super().__init__()
    ...
    def run(self) -> None:  # noqa: D401 - QThread entry point
        try:
            result = hydro_pipeline.run_hydro_forward(
                self._context, self._params, self._methods, self._p1, self._p2,
                log=lambda m: self.logged.emit(str(m), "info"),
            )
            self.succeeded.emit(result)
        except hydro_pipeline.BackendUnavailable as exc:
...
class HydroGeophysicsModule(BaseModule):
    ...
    def _select_folder(self) -> None:
        ...
        path = QFileDialog.getExistingDirectory(self, "Select hydro data folder", start)
        if path:
