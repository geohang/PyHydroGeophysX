...
from PyHydroGeophysX.qt_apps import geo_pipeline, io_utils, theme
...
class InverseWorker(QThread):
    """Runs the ERT -> water content / porosity Monte Carlo off the UI thread."""
    ...
    def run(self) -> None:  # noqa: D401 - QThread entry point
        try:
            result = geo_pipeline.run_ert_to_wc(
...
class GeoHydrologyModule(BaseModule):
    ...
    def __init__(self, state: Any, log: LogFn, parent=None) -> None:
        ...
        self._worker: Optional[InverseWorker] = None
        self._current = 0
    ...
    def _build_results_step(self) -> QWidget:
        ...
        self._gallery_layout.addWidget(
            QLabel("No results yet. Complete Steps 1–5 and run the estimation."))
    ...
    def _populate_results(self, result: Dict[str, Any]) -> None:
        ...
        summary = QLabel(
            ...
            f"<b>Products:</b> {', '.join(result.get('products', []))} &nbsp; "
        ...
        if not figs:
            self._gallery_layout.addWidget(
                QLabel("No figures were produced (config exported instead)."))
        for fig in figs:
            if Path(fig).exists():
                self._gallery_layout.addWidget(QLabel(f"<b>{Path(fig).name}</b>"))
                view = ZoomableImageView(); view.setMinimumHeight(400)
        ...
        if cfg:
            self._gallery_layout.addWidget(QLabel(f"Petrophysics config: <code>{cfg}</code>"))
    ...
    def _plot_points(self) -> None:
        ...
        try:
            ...
            return
        self._gallery_layout.addWidget(QLabel("<b>monitoring_series.png</b>"))
        view = ZoomableImageView(); view.setMinimumHeight(360)
        if view.set_image_file(str(out)):
            self._gallery_layout.addWidget(view)
    ...
    def _select_folder(self) -> None:
        ...
        path = QFileDialog.getExistingDirectory(self, "Select inverted-model folder", start)
        if path:
            self._manual_dir = Path(path)
            self._markers_override = None
            self._reload()
