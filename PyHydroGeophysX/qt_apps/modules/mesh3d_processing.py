...
from PyHydroGeophysX.qt_apps import io_utils, mesh3d_builder, theme
...
_INSTALL_MESSAGE = (
    ...
    "working <code>vtk</code> build. Mesh generation and export still work without it."
)
...
class Mesh3DModule(BaseModule):
    ...
    def _browse_output_dir(self) -> None:
        path = QFileDialog.getExistingDirectory(self, "Output directory", self._out_dir.text())
        if path:
    ...
    def _generate_mesh(self) -> None:
        ...
        worker = TaskWorker(mesh3d_builder.generate_mesh, cfg, with_log=True)
        worker.logged.connect(lambda m: self.log(m, "info"))
        worker.succeeded.connect(lambda res: self._on_mesh_generated(cfg, res))
