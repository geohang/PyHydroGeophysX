...
* ``build_3d_model`` -- the real run. It imports pygimli lazily to load the
  section meshes. PyVista is optional and adds VTK export plus a rendered 3D
  preview; the NumPy arrays and Matplotlib results are still built without it.
  3D velocity interpolation uses scipy ``griddata`` by default and ``gstools``
  ordinary kriging when that package is installed and requested.
"""
...
def _create_structured_grid(
    ...
    """Return an optional PyVista grid and its points.

    Grid coordinates are built with NumPy/SciPy so the scientific workflow does
    not require PyVista. When PyVista is installed, the equivalent structured
    grid is also returned for VTK export and rendering.
    """
    from scipy.interpolate import griddata

    x = np.linspace(topography[:, 0].min(), topography[:, 0].max(), grid_resolution)
    y = np.linspace(topography[:, 1].min(), topography[:, 1].max(), grid_resolution)
    xx, yy = np.meshgrid(x, y)
    zz = griddata(topography[:, :2], topography[:, 2], (xx, yy), method="nearest")
    shape = (xx.shape[0], xx.shape[1], len(z_cells))
    xx3 = np.repeat(xx[:, :, np.newaxis], len(z_cells), axis=2)
    yy3 = np.repeat(yy[:, :, np.newaxis], len(z_cells), axis=2)
    zz3 = np.repeat(zz[:, :, np.newaxis], len(z_cells), axis=2)
    zz3 -= np.cumsum(z_cells).reshape((1, 1, -1))
    points = np.column_stack([
        xx3.ravel(order="F"), yy3.ravel(order="F"), zz3.ravel(order="F")
    ])

    try:
        import pyvista as pv
    except Exception as exc:  # noqa: BLE001 - optional export/render dependency
        log(f"  pyvista unavailable; VTK export and volume rendering skipped: {exc}")
        return None, points

    grid = pv.StructuredGrid(
        xx3.reshape(shape), yy3.reshape(shape), zz3.reshape(shape))
    grid["Elevation"] = zz3.ravel(order="F")
    return grid, points
...
# ---------------------------------------------------------------------------
# Full 3D model build (requires pygimli; pyvista is optional)
# ---------------------------------------------------------------------------
def build_3d_model(
    ...
    grid, grid_points = _create_structured_grid(
        surface_pts, grid_res, _z_cells(depth, n_layers), log)
    ...
    vtk_path = None
    if grid is not None:
        candidate = out_dir / "seismic_3d_model.vtk"
        try:
            grid.save(str(candidate))
