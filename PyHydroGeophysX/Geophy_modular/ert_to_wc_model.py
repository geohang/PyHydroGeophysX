...
class ERTtoWC:
    ...
    def run_monte_carlo(
        ...
        for marker in self.layer_markers:
            distributions = self.layer_distributions[marker]
            layers.append({
                "marker": int(marker),
                "use_rho_sat": True,
                "rho_sat": distributions["rhos"],
                "n": distributions["n"],
                "sigma_sur": distributions["sigma_sur"],
                "porosity": distributions["porosity"],
            })
        result = run_petrophysics_monte_carlo(
            self.resistivity_values,
            self.cell_markers,
            layers,
        ...
        water_content_all = result["water_content_all"]
        saturation_all = result["saturation_all"]
        params_used = {}
        for marker, values in result["params_used"].items():
            params_used[marker] = {
                "rhos": values["rho_sat"],
                "n": values["n"],
                "sigma_sur": values["sigma_sur"],
    ...
    def plot_water_content(self, time_idx: int = 0, ax=None, 
        ...
        try:
            ...
            raise optional_import_error("ERTtoWC.plot_water_content", exc) from exc
