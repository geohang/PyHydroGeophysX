...
def _run_monte_carlo(
    ...
    """Compatibility wrapper around the canonical seeded implementation."""
...
def run_ert_to_wc(
    ...
    def _mesh_map(values: np.ndarray, title: str, label: str, fname: str,
                  cmap: str, vmin: float, vmax: float) -> str:
    ...
    if "water_content" in stats:
        ...
        display_paths["water_content"] = {}
        for key in ("mean", "std", "p10", "p50", "p90"):
            values = wc[key][:, preview_col]
            is_std = key == "std"
            title = {
                "mean": "mean", "std": "standard deviation",
                "p10": "P10", "p50": "P50 (median)", "p90": "P90",
            }[key]
            vmax = float(max(1e-3, np.nanmax(values))) if is_std else cmax
            display_paths["water_content"][key] = _mesh_map(
                values,
                f"Water content ({title}), t={preview_t}",
                "Std water content (-)" if is_std else "Water content (-)",
                f"wc_{key}_map.png",
                "magma" if is_std else "viridis",
                0.0 if is_std else cmin,
                vmax,
            )
    ...
    if "porosity" in stats:
        por = stats["porosity"]
        display_paths["porosity"] = {}
        for key in ("mean", "std", "p10", "p50", "p90"):
            values = por[key][:, preview_col]
            is_std = key == "std"
            title = {
                "mean": "mean", "std": "standard deviation",
                "p10": "P10", "p50": "P50 (median)", "p90": "P90",
            }[key]
            vmax = float(max(1e-3, np.nanmax(values))) if is_std else 0.5
            display_paths["porosity"][key] = _mesh_map(
                values,
                f"Porosity ({title}), t={preview_t}",
                "Std porosity (-)" if is_std else "Porosity (-)",
                f"porosity_{key}_map.png",
                "magma" if is_std else "viridis",
                0.0,
                vmax,
            )
