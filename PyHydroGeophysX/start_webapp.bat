...
title PyHydroGeophysX Web Interface
...
set "APP_FILE=%REPO_DIR%\examples\app_geophysics_workflow.py"
set "VENV_DIR=%REPO_DIR%\.venv-webapp"
set "PYTHON_EXE="
set "PYTHON_ARGS="
set "FALLBACK_EXE="
set "FALLBACK_ARGS="
...
if not exist "%APP_FILE%" (
    ...
    echo         "%APP_FILE%"
    echo.
    echo Keep start_webapp.bat in the PyHydroGeophysX package folder of the
...
rem Streamlit was not found, but a suitable Python may be available. Create an
...
echo.
"%VENV_DIR%\Scripts\python.exe" -m pip install --disable-pip-version-check -e "%REPO_DIR%[webapp]"
if errorlevel 1 (
    echo.
...
if /i "%PHGX_WEBAPP_DRY_RUN%"=="1" exit /b 0

"%PYTHON_EXE%" %PYTHON_ARGS% -m streamlit run "%APP_FILE%" --server.port 8501 --server.headless false --browser.gatherUsageStats false
if errorlevel 1 (
    echo.
    echo [ERROR] The web server stopped unexpectedly. See the message above.
    goto :failed
)
exit /b 0
