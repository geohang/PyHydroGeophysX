...
echo "== PyHydroGeophysX workbench build: $variant =="
...
pyinstaller --noconfirm \
  --distpath "$repo/dist" \
  --workpath "$repo/build" \
  "$repo/packaging/pyinstaller_workbench.spec"

os_name="linux"
if [ "$(uname -s)" = "Darwin" ]; then os_name="macos"; fi
out="$repo/dist/PyHydroGeophysX-Workbench-${os_name}-${variant}.zip"
rm -f "$out"

if [ "$os_name" = "macos" ] && [ -d "$repo/dist/PyHydroGeophysX-Workbench.app" ]; then
  # ditto preserves the .app structure, symlinks, and permissions
  ditto -c -k --keepParent "$repo/dist/PyHydroGeophysX-Workbench.app" "$out"
else
  (cd "$repo/dist" && zip -qr "$(basename "$out")" "PyHydroGeophysX-Workbench")
fi

echo "Built $out"
echo "Smoke test: QT_QPA_PLATFORM=offscreen \"$repo/dist/PyHydroGeophysX-Workbench/PyHydroGeophysX-Workbench\" --self-test"
